...
path=: jpath '~temp'
NB. dll=:'"',path,'lbfgs.dll" '
3 : 0''
if. UNAME-:'Linux' do.
  DLL=: jpath'~temp/lbfgs.so'
...
if. > {. r do.
  error x;'lbfgs dll return code: ',": > {. r
else.
  LASTOUT=: }.r
end.
)
...
NB. =========================================================
NB. lbfgs_test - run test case using the lbgfs routine.
lbfgs_test=: 3 : 0
NDIM=. 2000
MSAVE=. 7
...
while. 1 do.
 ...
 'N M X F G DIAGCO DIAG IPRINT EPS XTOL W IFLAG'=.r=.' lbfgs_' call (N;M;X;F;G;DIAGCO;DIAG;IPRINT;EPS;XTOL;W;IFLAG)
